...
function HomePage(props) {
  ...
  const calcBeerFactor = () => {
    ...
    if (salary && responseData) {
      let beerFactor1 =
        parseInt(salary[0]["Annual"].replace(/,/g, "")) /
        parseInt(responseData["Restaurants prices"][3]["Value"]);
      ...
      <div className="App">
        ...
        <div>
          ...
          <GoButton
            ...
            onResponse={handleButtonResponse}
          />
          {responseData && <DisplayTable responseObject={responseData} />}

          <CitySelect onCitySelected={handleCitySelected2} />
          <CompareButton
            selectedCity={selectedCity2}
            onResponse={handleButtonResponse2}
          />
          {responseData2 && <DisplayTable responseObject={responseData2} />}
        </div>
        <div>
        ...
        <div>
          {" "}
          <JobSelect onJobSelected={handleJobSelected} />
        </div>
        <SelectJobButton
          selectedJob={selectedJob}
          onResponse={handleSelectJobButtonResponse}
        />

        {beerFactor && (
          <p>
            The beer factor in your selected state is: {beerFactor} with a an
            average salary of {salary[0]["Annual"]}{" "}
          </p>
        )}
        <div>
